...
const STRIPE_SECRET_KEY = "sk_test_51T5s7BD4EJNMKSAqrgqHsVeCc1qjn1Cx25OZ4io2cIZlB3mPF1khTlVQCQx1Odbh62NqJtWgN6r9WbHhgWmcLrZ700iQymyoRa";
...
export async function getBooking(req, res) {
    try {
        if(!req.user) return res.status(401).json({ success: false, message: 'Authentiction required.'});
...
// confirm payment 
   export async function confirmPayment(req, res) {
        try {
            ...
            let stripe;
            try{stripe = getStripeOrThrow();} catch(err){
                return res.status(500).json({ success: false , message: 'Payment not configure', error: err.mesage});
            }

            const sessionObj = await stripe.checkout.sessions.retrieve(session_id);
            if(!sessionObj) return res.status(404).json({ success: false, message: 'Failed to find session' });
