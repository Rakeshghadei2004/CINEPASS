...
import { Clock, Film, Image as ImageIcon, Play, Plus, Star, Users, X, Upload, } from 'lucide-react';
...
const AddPage = () => {
  ...
  //to calculate total
  useEffect(() => {
    const total = (Number(durationHours) || 0) * 60 + (Number(durationHours) || 0);
    setDuration(total);
  ...
  //it reset the field once the data get submited to the server
  function resetForm() {
    setMovieName("");
    setCategories([]);
    setPoster(null);
  ...
  //this functio validate all the field are filled or not with correct details
  function validate() {
  ...
  //this function helps to create movie
  async function handleSubmit(e) {
    ...
    // if movieType === "latestTrailers" then it will create a latesttrailerObj
    if (movieType === "latestTrailers") {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || error.message || 'Failed to upload.';
      toast.error(msg);
    } finally {
          ...
          items.map((it, idx) => (
            <div key={idx} className={addMoviePageStyles.uploaderItem}>
              ...
              {typeof it.name !== 'undefined' && (
                <div className='mt-2'>
                  <input value={it.name} onChange={(e) => updateMeta && updateMeta(idx, "name", e.target.value)} placeholder='Name' className={addMoviePageStyles.uploaderItemInput} />
                </div>
              )}

              {typeof it.role !== 'undefined' && (
                <div className='mt-2'>
                  <input value={it.role} onChange={(e) => updateMeta && updateMeta(idx, "role", e.target.value)} placeholder='Role' className={addMoviePageStyles.uploaderItemInput} />
                </div>
              )}
            </div>
          ))
...
//for latest Trailers we NamedUploader
          ...
          items.map((it, idx) => (
            <div key={idx} className={addMoviePageStyles.namedUploaderItem}>
              ...
              <div className='flex-1'>
                <input value={it.name} onChange={(e) => updateName(idx, e.target.value)} placeholder='Name' className={addMoviePageStyles.namedUploaderInput} />
